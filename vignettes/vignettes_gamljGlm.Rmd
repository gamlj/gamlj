...
The same analysis can be done by updating the model with the `gamlj_update()` function. Almost all the options available in `gamljGlm()` can be added to a model by running `gamlj_update(mod,...)` where `...` is any option or options accepted by `gamljGlm()`.
...
Any plot produced by `gamljGlm` can be obtained as a ggplot2 object for futher manipolations or usage. For instance, one can change the theme of the plot:

```{r warning=FALSE, message=FALSE}
myplot<-gamlj_ggplot(mod4)
