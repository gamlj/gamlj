...
names64 <- R6::R6Class("names64", list(.factors = list(), .covs = list(), .labels = list(), .contrasts = list(), addFactor = function(variable, 
    levels) {
    ...
    res <- astring
    for (a in names(self$.factors)) res <- gsub(a, self$.factors[[a]], res)
    for (a in names(self$.covs)) res <- gsub(a, self$.covs[[a]], res)
    return(res)
    ...
    if (length(x) == 2) return(paste0(jmvcore::fromB64(x[1]), x[2]))
    ### then we search for names
    if (var64 %in% names(self$.factors)) return(self$.factors[[var64]])
    if (var64 %in% names(self$.covs)) return(self$.covs[[var64]])
    return(var64)
}, .onelabel = function(obj) {

    if (obj %in% names(self$.labels)) return(self$.labels[[obj]])
    if (obj %in% names(self$.covs)) return(self$.covs[obj])
