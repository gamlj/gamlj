...
gsimple.populate <- function(model, options, tables, cov_conditioning) {
    ...
    if (!is.null(anovaTable$state)) {
        ...
        simple_test <- try({
            resultsTables <- pred.simpleEstimates(model, jmvcore::toB64(variable), jmvcore::toB64(moderator), jmvcore::toB64(threeway), 
    ...
    } else parametersTable$getColumn("z.ratio")$setVisible(FALSE)
    ...
    if (!is.null(term)) {
        if (mi.is.scaleDependent(model, term)) 
            parametersTable$setNote("inter", WARNS["se.interactions"]) else if (mi.term.develop(term) < length(options$modelTerms)) 
            parametersTable$setNote("covs", WARNS["se.covariates"])
    } else parametersTable$setNote("noint", WARNS["se.noint"])
}
