...
.range.default <- function(model, depName, predictions, rawData = NULL, linearPred = FALSE) {
    ...
    if ("lwr" %in% names(predictions)) 
        yAxisRange <- pretty(c(predictions$lwr, predictions$upr, rawRange)) else yAxisRange <- pretty(c(predictions$fit, rawRange))
...
.rawData.default <- function(model, depName, groupName, linesName = NULL, plotsName = NULL) {
    ...
    if (is.null(linesName) || !is.factor(data[[jmvcore::toB64(linesName)]])) 
        data$z <- 1 else data$z <- data[, jmvcore::toB64(linesName)]

    if (is.null(plotsName) || !is.factor(data[[jmvcore::toB64(plotsName)]])) 
        data$w <- 1 else data$w <- data[, jmvcore::toB64(plotsName)]
...
.preparePlotData.default = function(model, groupName, linesName = NULL, plotsName = NULL, bars = "none", ciwidth = 95, conditioning = NULL, 
    type = "response") {
    ...
    if (bars == "none") {
        pdata$lwr <- NULL
        pdata$upr <- NULL
    } else if ("glm" %in% class(model) && "binomial" %in% stats::family(model)["family"]) {
...
.preparePlotData.multinom = function(model, groupName, linesName = NULL, plotsName = NULL, bars = "none", ciwidth = 95, conditioning = NULL) {
    ...
    if ("plots2" %in% names(pdata)) 
        if (is.factor(data[[jmvcore::toB64(linesName)]])) 
            pdata$plots2 <- factor(pdata$plots2, levels = levels(data[[jmvcore::toB64(linesName)]])) else pdata$plots2 <- factor(pdata$plots2, levels = unique(pdata$plots2), ordered = TRUE)
...
gplots.images <- function(self, data, raw, range, randomData = NULL) {
    ...
    if (is.null(plotsName)) {
        ...
        for (i in seq_along(glevels)) {
            ...
            if (!is.null(raw)) {
                if (is.factor(raw[["w"]])) 
                  sraw <- raw[raw$w == levels[i], ] else sraw <- raw
            }
            srand <- NULL
...
gplots.initPlots = function(obj, data, cov_condition) {
    ...
    if (isMulti) {
        ...
        if (is.factor(sepPlotsVar)) 
            sepPlotsLevels <- levels(sepPlotsVar) else sepPlotsLevels <- cov_condition$labels(sepPlotsName)
...
#### 'image' should be from jamovi image object this function assumes that the y is called 'mean', the x is 'group' and the tracing factor
#### 'lines' (if any) errorType is an option for plotting error bars or CI. It can either '' (empty) which means no bars or CI or any other
#### string. The string will be used as the legend name and the function assumes that the data contains 'lwr' and 'upr' varianbles 'theme'
#### is passed from jmv plot function
...
gplots.twoWaysPlot <- function(image, theme, depName, groupName, linesName, errorType = "none", title = NULL, order = 1) {
    ...
    if (!is.null(image$state$raw)) {
        ...
        if (is.factor(rawData$z)) 
            p <- p + ggplot2::geom_point(data = rawData, aes_string(x = "x", y = "y", color = "z"), show.legend = FALSE, alpha = 0.5, shape = 16, 
    ...
    if (is.factor(image$state$data$group)) {
        ...
        p <- p + ggplot2::geom_point(data = gdata, aes_string(x = "group", y = "fit", group = "lines", colour = "lines"), shape = 16, size = 4, 
            position = dodge)
        if (errorType != "") 
            p <- p + ggplot2::geom_errorbar(data = gdata, aes_string(x = "group", ymin = "lwr", ymax = "upr", width = 0.1, group = "lines", 
                color = "lines"), size = 1.2, position = dodge)
        ...
        if (!is.null(image$state$randomData)) {
            ...
            p <- p + ggplot2::geom_line(data = data, aes_string(x = "group", y = "y", group = "cluster"), alpha = 0.5, size = 0.2, color = "cadetblue3", 
                show.legend = F)
        }
        if (errorType != "") 
            p <- p + ggplot2::geom_ribbon(data = gdata, aes_string(x = "group", ymin = "lwr", ymax = "upr", group = "lines", colour = "lines", 
                fill = "lines"), linetype = 0, show.legend = F, alpha = 0.2)
...
gplots.oneWayPlot <- function(image, theme, depName, groupName, errorType = "none", order = 1) {
    ...
    p <- ggplot2::ggplot(data = image$state$data) + ggplot2::labs(x = groupName, y = depName, colour = clabel) + ggplot2::scale_y_continuous(limits = c(min(image$state$range), 
        max(image$state$range)))

    if (!is.null(image$state$raw)) {
        rawData = image$state$raw
        p <- p + ggplot2::geom_point(data = rawData, aes_string(x = "x", y = "y"), colour = "cadetblue3", shape = 16, alpha = 0.5, size = 2)
    }
    ...
    if (is.factor(image$state$data$group)) {
        if (!is.null(image$state$randomData)) {
            ...
            p <- p + ggplot2::geom_point(data = data, aes_string(x = "group", y = "y", group = "cluster"), show.legend = F, shape = 12, 
                alpha = 0.5, size = 0.5)
            p <- p + ggplot2::geom_line(data = data, aes_string(x = "group", y = "y", group = "cluster"), size = 0.3, alpha = 0.3, show.legend = F)
        }

        if (errorType != "") {
            p <- p + ggplot2::geom_errorbar(data = gdata, aes_string(x = "group", ymin = "lwr", ymax = "upr"), width = 0.1, size = 0.8, 
                show.legend = F)
        }
        ...
        if (!is.null(image$state$randomData)) {
            ...
            p <- p + ggplot2::geom_line(data = data, aes_string(x = "group", y = "y", group = "cluster"), alpha = 0.5, size = 0.2, show.legend = F)
        }

        if (errorType != "") 
            p <- p + ggplot2::geom_ribbon(aes_string(x = "group", ymin = "lwr", ymax = "upr"), show.legend = F, alpha = 0.2)
...
gplots.linesMultiPlot <- function(image, ggtheme, depName, groupName, linesName = NULL, plotsName = NULL, errorType = "none", title = NULL) {
    ...
    if (!is.null(linesName)) {
        ...
        return(plots)
    } else return(gplots.twoWaysPlot(image, ggtheme, depLabs, groupName, depName, errorType = errorType) + ggtheme)
}
...
# .normPlot=function(image, ggtheme, theme) { model<-private$.model if (is.null(model) ) return(FALSE) fill <- theme$fill[2] color <-
# theme$color[1] alpha <- 0.4 data<-data.frame() data <- as.data.frame(stats::rstandard(model)) names(data)<-'x'
# ndata<-as.data.frame(stats::rnorm(1000,mean(data$x),stats::sd(data$x))) names(ndata)<-'norm' # library(ggplot2) plot <-
# ggplot(data=data, aes(x=x)) + labs(x='Residuals', y='density') plot <- plot + geom_histogram(aes(y=..density..), position='identity',
# stat='bin', color=color, fill=fill) plot <- plot + stat_function(fun = stats::dnorm, args = list(mean = mean(data$x), sd =
# stats::sd(data$x))) themeSpec <- theme(axis.text.y=element_blank(), axis.ticks.y=element_blank()) }
...
gplots.normPlot = function(model, ggtheme, theme) {
    ...
    plot <- plot + geom_histogram(aes_string(y = "..density.."), position = "identity", stat = "bin", color = color, fill = fill)
