...
.fixLabels <- function(table, preds, cov_conditioning) {
    ...
    for (p in preds) {
        if (p %in% jmvcore::toB64(cov_conditioning$vars)) 
            aList[[p]] <- cov_conditioning$labels(p, decode = T) else {
...
#### simple Effects estimates: parameters table and anova table conditioning expects an object of R6 class 'conditioning'
...
.rawMeans.lmerModLmerTest <- function(model, terms, cov_conditioning = conditioning$new(), interval = 95, type = "link", df = "Satterthwaite") .rawMeans.merModLmerTest(model, 
    terms, cov_conditioning, df)
...
pred.means <- function(model, terms, cov_conditioning = conditioning$new(), interval = 95, type = "response") {
    suppressWarnings(est <- rawMeans(model, terms, cov_conditioning, interval = interval, type = type))
    ...
    for (nn in names(dd)) if (is.factor(dd[[nn]])) 
...
.simpleEstimates.glmerMod <- function(model, variable, moderator, threeway = NULL, cov_conditioning = conditioning$new(), interval = 95) {
    .simpleEstimates.glm(model, variable, moderator, threeway = threeway, cov_conditioning = cov_conditioning, interval = interval)
}

.simpleEstimates.glm <- function(model, variable, moderator, threeway = NULL, cov_conditioning = conditioning$new(), interval = 95) {

    tables <- .simpleEstimates.default(model, variable, moderator, threeway = threeway, cov_conditioning = cov_conditioning, interval = interval)
    params <- tables[[1]]
...
.simpleEstimates.default <- function(model, variable, moderator, threeway = NULL, cov_conditioning = conditioning$new(), interval = 95) {
    ...
    if (is.factor(data[[variable]])) {
        emm <- rawMeans(model, unlist(c(variable, moderator, threeway)), cov_conditioning = cov_conditioning, interval = interval)
        ...
        lev <- interval/100
        # est<-emmeans::emtrends(model,specs=preds,var=variable,at=condlist,options=list(level=lev)) emmeans::emm_options(ref_grid = list(level
        # = .90))
...
.simpleEstimates.multinom <- function(model, variable, moderator, threeway = NULL, cov_conditioning = conditioning$new(), interval = 95) {
    ...
    params <- data.frame()
    ######## for multinom emmeans::test does not work well, so we use the centering method
    .vals <- list()
    .levs <- list()
    for (pred in preds) if (pred %in% jmvcore::toB64(cov_conditioning$vars)) {
    ...
    for (l in 1:nrow(vals)) {
        .data <- data
        for (name in preds) if (is.factor(.data[[name]])) 
            stats::contrasts(.data[[name]]) <- stats::contr.treatment(length(unique(vals[[name]])), base = vals[[name]][l]) else .data[[name]] <- .data[[name]] - vals[[name]][l]
...
pred.predictRandom <- function(model, dep64, groupName64, cluster64, linesName64 = NULL, plotsName64 = NULL, type = "response") {

    data <- model@frame
    preds64 <- c(groupName64, linesName64, plotsName64)
    gclass <- class(data[[groupName64]])
    # here we set all model predictors but the x-axis variable to zero to smooth random effects predicted values
    mvars <- names(data)
    tozero <- setdiff(mvars, c(groupName64, cluster64, dep64))
    newdata <- data
    for (v in tozero) if (!is.factor(newdata[, v])) 
        newdata[, v] <- 0
...
.predictRandom_continuous <- function(model, groupName, cluster, linesName = NULL, plotsName = NULL, type = "response") {
